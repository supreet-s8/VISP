...
sub collector {
	...
	foreach my $type (keys %$adaptor) {
		...
		# ----------------------------------------------
			...
			#"internal set modify - /pm/process/allothttphelper/launch_params/1 value uint8 1",
			#"internal set modify - /pm/process/allotpphelper/launch_params/1 value uint8 1",
			#"pm process allotpphelper launch auto",
			#"pm process allotpphelper launch relaunch auto",
			#"pm process allotpphelper launch params 2 -s",
			#"pm process allotpphelper launch params 3 /data/pnsa/pilot/",
			#"pm process allotpphelper launch params 4 -d",
			#"pm process allotpphelper launch params 5 /data/pnsa/radiusdata/",
			#"pm process allothttphelper launch auto",
			#"pm process allothttphelper launch relaunch auto",
			#"pm process allothttphelper launch params 2 -s",
			#"pm process allothttphelper launch params 3 /data/pnsa/http/",
			#"pm process allothttphelper launch params 4 -d",
			#"pm process allothttphelper launch params 5 /data/pnsa/ipfixdata/",
			#"pm process allotpphelper restart",
			#"pm process allothttphelper restart"
                );
...
sub oozie {
	...
	push (@cmds,	"pmx subshell oozie set job PnsaSubscriberIb action SubscriberIBCreator attribute binaryInput true",
			...
			"pmx subshell oozie set job PnsaSubscriberIb action SubscriberIBCreator attribute inputDatasets pnsa_selfipcleanup",
			...
			"pmx subshell oozie set job Ipfixcp action DistcpAction attribute destNamenode $$commonAttributes{destNamenode}",
			"pmx subshell oozie set job Ipfixcp action DistcpAction attribute retryCount 5",
			"pmx subshell oozie set job Ipfixcp action DistcpAction attribute srcNamenode $$commonAttributes{srcNamenode}", 
			...
			"pmx subshell oozie set job Radiuscp action DistcpAction attribute retryCount 5",
	...
	push (@cmds,	"pmx subshell oozie add dataset pnsa_selfipcleanup",
